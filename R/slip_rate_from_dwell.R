...
exact_slip_rate_from_dwell<-function(dwell_model_all_df,screen_interval=1,weibull_shape=1){
  ...
  #select a scenario as a template
  select_one_scenario<-dwell_slip_df %>% pull(scenario) %>% unique()
  
  #now manufacture maximum-interception by never missing
  dwell_ideal_df <- dwell_slip_df %>%
    filter(scenario==select_one_scenario[1]) %>%
    mutate(dwell=10000,slip=0,slip_clinical=0,scenario="MIS",screen_interval=NA)
  
  #add "scenario MIS" perfect interception
